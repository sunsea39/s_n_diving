...
create policy "board images delete own or admin" on storage.objects for delete to authenticated using (bucket_id = 'board-images' and (owner_id = (select auth.uid()) or public.is_admin()));
